// app/login/page.js - REX LOGIN PAGE (RESPONSIVO MOBILE)
'use client';
        ...
        <div className="text-center">
          <div className="relative w-20 h-20 sm:w-32 sm:h-32 mx-auto mb-6 sm:mb-8">
            <Image
              ...
              width={128}
              height={128}
              className="animate-spin drop-shadow-2xl w-full h-full"
              priority
            />
          </div>
          <p className="text-yellow-400 font-mono text-sm sm:text-lg font-bold tracking-wider animate-pulse">
            CARREGANDO REINO...
          </p>
        </div>
      </div>
    ...
    <>
      ...
      <style jsx>{`
        @keyframes mystical-glow {
          0%, 100% { 
            box-shadow: 0 0 20px rgba(251, 191, 36, 0.15), inset 0 0 15px rgba(251, 191, 36, 0.03);
          }
          50% { 
            box-shadow: 0 0 35px rgba(251, 191, 36, 0.3), inset 0 0 25px rgba(251, 191, 36, 0.08);
          }
        }
      ...
      >
        ...
        <canvas 
          ref={canvasRef}
          className="absolute inset-0 w-full h-full pointer-events-none opacity-20 sm:opacity-30"
        />
        
        {/* Glow central */}
        <div className="absolute inset-0 bg-radial-gradient from-yellow-500/5 via-transparent to-transparent sm:from-yellow-500/10"></div>
        
        {/* Link de volta */}
        <div className="back-link absolute top-4 left-4 sm:top-6 sm:left-6 z-20" style={{opacity: 0, transform: 'translateY(20px)'}}>
          <Link 
            href="/" 
        ...
        {/* Container principal */}
        <div className="relative min-h-screen flex flex-col items-center justify-center px-3 sm:px-4 py-4 sm:py-6">
          
          {/* Logo REX */}
          <div className="logo-login text-center mb-4 sm:mb-8" style={{opacity: 0, transform: 'translateY(80px) scale(0.9)'}}>
            <Image
              src="/logorex.png"
              alt="REX - Regions Exchange"
              width={640}
              height={240}
              className="h-32 w-auto xs:h-36 sm:h-48 md:h-56 lg:h-64 xl:h-72 drop-shadow-2xl mx-auto"
              priority
            />
          </div>
          
          {/* Portal de Login */}
          <div className="login-portal relative w-full max-w-xs xs:max-w-sm sm:max-w-md" style={{opacity: 0, transform: 'translateY(40px) scale(0.95)'}}>
            ...
            <div className="relative stone-texture backdrop-blur-sm rounded-2xl sm:rounded-3xl border-2 border-yellow-500/30 mystical-glow overflow-hidden">
              ...
              {/* Conteúdo do portal */}
              <div className="relative z-20 p-4 xs:p-6 sm:p-8 md:p-10">
                ...
                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                  ...
                  {/* Mensagem de erro */}
                  {error && (
                    <div className="bg-red-900/50 border border-red-500/50 rounded-lg sm:rounded-xl p-2 sm:p-3">
                      <p className="text-red-400 text-xs sm:text-sm font-mono text-center">{error}</p>
                    </div>
                  )}
                    ...
                    >
                      {isSubmitting ? (
                        <div className="flex items-center justify-center">
                          ...
                          <span className="text-xs sm:text-sm">ACESSANDO REINO...</span>
                        </div>
                      ) : (
                        <span className="text-xs sm:text-sm">ENTRAR NO REINO</span>
                      )}
                    </button>
                  </div>
                  ...
                  <div className="form-field flex items-center" style={{opacity: 0, transform: 'translateY(20px)'}}>
                    ...
                    <span className="px-3 sm:px-4 text-gray-400 font-mono text-xs">OU</span>
                    <div className="flex-1 h-px bg-gradient-to-r from-transparent via-yellow-500/30 to-transparent"></div>
                  </div>
                  ...
                  <div className="form-field" style={{opacity: 0, transform: 'translateY(20px)'}}>
                    <button
                      ...
                      disabled={isSubmitting}
                      className="w-full bg-white hover:bg-gray-50 disabled:bg-gray-300 text-gray-800 font-mono font-bold py-3 sm:py-4 px-4 sm:px-6 rounded-lg sm:rounded-xl transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:shadow-white/20 disabled:scale-100 disabled:hover:shadow-none border-2 border-gray-300 hover:border-gray-400 flex items-center justify-center text-sm sm:text-base"
                    >
                      {isSubmitting ? (
                        <div className="flex items-center justify-center">
                          <div className="w-4 h-4 sm:w-5 sm:h-5 border-2 border-gray-800 border-t-transparent rounded-full animate-spin mr-2"></div>
                        ...
                        <>
                          ...
                          <span className="text-xs sm:text-sm">ENTRAR COM GOOGLE</span>
                        </>
                      )}
                    </button>
                  </div>
                  ...
                  {/* Links adicionais */}
                  <div className="form-field text-center pt-2 sm:pt-4" style={{opacity: 0, transform: 'translateY(20px)'}}>
